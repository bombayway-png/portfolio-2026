...
export default function Home() {
  ...
  // Auto-resizing logic for high-detail technical input
  useEffect(() => {
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100 overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 h-20 flex items-center px-4 md:px-8">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center text-slate-900">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            <a href="https://www.linkedin.com/in/adamseumae/" target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-blue-600 transition-colors"><Linkedin size={20} /></a>
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-32 md:pt-48 pb-24">
        ...
        <section className="flex flex-col md:flex-row gap-12 md:gap-16 items-center mb-16 md:mb-24 text-center md:text-left">
          <div className="flex-1 order-2 md:order-1">
            ...
            <p className="text-lg md:text-2xl text-slate-500 max-w-3xl font-medium italic mb-10 leading-relaxed mx-auto md:mx-0">
              Bridging the gap between Enterprise Strategy and Code. Leveraging 10+ years at <strong>Amazon, AWS, Microsoft, and Blizzard</strong> to build autonomous frameworks.
            </p>
        ...
        <section className="mb-24 md:mb-40 grid grid-cols-1 md:grid-cols-3 gap-8">
            <TriadCard icon={LayoutDashboard} title="UX Architecture" desc="Bridging complex backends with intuitive frontends, forged at Blizzard." color="bg-blue-50 text-blue-600" />
            <TriadCard icon={Server} title="Core Infrastructure" desc="Architecting frameworks that eliminate technical debt at AWS scale." color="bg-slate-900 text-white" />
            <TriadCard icon={Bot} title="Agentic Intelligence" desc="Deploying autonomous agents that plan and act without supervision." color="bg-blue-600 text-white" />
        </section>

        {/* --- Pillars --- */}
        <ServicePillarSection title="Pillar 1: App Development" pitch="I engineer reactive, high-performance frontends." proofTitle="Feature Owner @ Blizzard" proofBody={<>Delivered gamepad support for <strong>battle.net on Xbox</strong>.</>} capabilities={[{title: "Real-Time:", desc: "Firebase listeners for instant UI updates."}]} corporateScale="Ownership of high-traffic UX for Call of Duty." icon={Code2} visualColor="text-blue-400" />
        
        <ServicePillarSection title="Pillar 2: Backend Architecture" pitch="I build the Central Nervous System of your business." proofTitle="Ops Excellence @ AWS" proofBody={<>Engineered internal solutions to avoid <strong>$6M in contract renewals</strong>.</>} capabilities={[{title: "Security:", desc: "Zero-trust serverless backend logic."}]} corporateScale="Benchmarking automation across 15 international locales." icon={Database} visualColor="text-slate-400" />

        {/* --- Audit Offer --- */}
        <section className="bg-blue-600 rounded-[3rem] p-12 md:p-20 text-center text-white mb-24 shadow-2xl">
          <h2 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter mb-8">Free Technical Audit</h2>
          <p className="text-xl md:text-2xl font-medium italic leading-relaxed max-w-4xl mx-auto mb-12 opacity-90">If I can&apos;t find a way to use <strong>Agentic AI</strong> to reduce your manual workload by <strong>20%</strong>, the consultation is free.</p>
          <button onClick={() => openPortal('AI Strategy Consult')} className="bg-white text-blue-600 px-10 py-5 rounded-full text-xl font-black hover:scale-105 transition-all italic uppercase">Claim My Audit</button>
        </section>
      </main>

      {/* --- Dynamic Unified Portal --- */}
        ...
        {activeFilter && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-white p-4 md:p-6 overflow-y-auto">
            <div className="max-w-4xl mx-auto py-12 md:py-20 relative text-left">
              ...
              {activeFilter === 'intake' ? (
                <motion.div key="intake-step" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-slate-900">
                  ...
                  <form className="space-y-8 md:space-y-12">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                        <div className="space-y-3 md:col-span-2 relative">
                           ...
                           <div className="relative"><select className={`${inputClasses} cursor-pointer hover:border-blue-400`} value={formData.projectType} onChange={(e) => setFormData({...formData, projectType: e.target.value})}><option value="" disabled>Select Objective</option><option value="AI Strategy Consult">AI Strategy Consult</option><option value="First AI Agent Build">First AI Agent Build</option><option value="Multi-Agent Workforce">Multi-Agent Workforce</option><option value="High-Fidelity Application">High-Fidelity Application</option></select><ChevronDown className="absolute right-0 bottom-4 text-slate-400 pointer-events-none" size={24} /></div>
                        </div>
                        ...
                        <div className="space-y-3 md:col-span-2"><label className="text-[10px] md:text-xs font-black uppercase tracking-widest text-blue-600">Desired Outcome</label><textarea ref={outcomeRef} className={`${inputClasses} resize-none overflow-hidden`} rows={1} placeholder="Primary Goal" value={formData.outcome} onChange={(e) => setFormData({...formData, outcome: e.target.value})} /></div>
                ...
                <motion.div key="review-step" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-left space-y-12">
                  <div className="bg-slate-900 text-white p-8 md:p-12 rounded-[3rem] shadow-2xl border-4 border-blue-600 relative overflow-hidden">
                    ...
                    <div className="space-y-8 relative z-10 uppercase italic">
                      ...
                      <div><p className="text-xs font-black text-slate-500 tracking-widest mb-1">Project Intent</p><p className="text-xl font-bold">{formData.projectType}</p></div>
                      ...
                      <div className="pt-6 border-t border-slate-800 flex items-center gap-2 text-blue-400 font-bold"><Zap size={18} /> Ready for Agentic Synthesis</div>
                    </div>
                  </div>
                  <div className="space-y-6 text-slate-900">
                    <h3 className="text-2xl font-black italic uppercase">Synchronize Discovery</h3>
                    <p className="text-lg text-slate-500 italic font-medium leading-relaxed max-w-2xl">By approving, these briefing requirements will be injected into our architecture session invitation.</p>
    ...
    <section className="bg-slate-50 rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-16 mb-16 md:mb-24 relative overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start relative z-10">
        <div className="space-y-8 order-2 lg:order-1">
          <div><h2 className="text-sm font-black uppercase tracking-[0.3em] text-blue-600 mb-4 italic">{title}</h2><p className="text-2xl md:text-4xl font-black italic text-slate-900 leading-tight">&quot;{pitch}&quot;</p></div>
