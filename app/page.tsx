...
import { 
  ...
  Server, Bot, ChevronDown, CheckCircle, Calendar, ArrowRight 
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100 overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-32 md:pt-48 pb-24">
        ...
        <section className="bg-blue-600 rounded-[3rem] p-12 md:p-20 text-center text-white mb-24 shadow-2xl">
          ...
          <button onClick={() => openPortal('An AI Strategy Consult')} className="bg-white text-blue-600 px-10 py-5 rounded-full text-xl font-black hover:scale-105 transition-all shadow-2xl italic uppercase">
            Schedule a Call
          </button>
        </section>
      </main>
        ...
        {activeFilter === 'intake' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-white p-4 md:p-6 overflow-y-auto">
            <div className="max-w-4xl mx-auto py-12 md:py-20 relative text-left">
              ...
              {!submitted ? (
                <motion.div key="form-view" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}>
                  ...
                  <form className="space-y-8 md:space-y-12">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 text-slate-900">
                      <div className="space-y-3 md:col-span-2 relative">
                        ...
                        <div className="relative group">
                          <select className={`${inputClasses} cursor-pointer hover:border-blue-400`} value={formData.projectType} onChange={(e) => setFormData({...formData, projectType: e.target.value})}>
                    ...
                    <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-blue-600 text-white py-6 md:py-8 rounded-2xl md:rounded-[2rem] text-xl md:text-3xl font-black italic uppercase shadow-2xl hover:scale-[1.02] transition-all flex items-center justify-center gap-4">
                      {isSubmitting ? "Synthesizing Strategy..." : "Begin Architecture"}
                ...
                <motion.div key="success-view" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-12 md:py-24 space-y-12">
                  ...
                  <div className="space-y-4">
                    <h2 className="text-4xl md:text-7xl font-black italic uppercase text-slate-900">Strategy Inbound</h2>
                    <p className="text-xl md:text-2xl font-medium italic text-slate-500 max-w-2xl mx-auto">
                  ...
                  <div className="bg-slate-50 rounded-[3rem] p-8 md:p-12 border-2 border-dashed border-slate-200 inline-block w-full">
                    ...
                    <a href="https://calendly.com/adamseumae" target="_blank" rel="noopener noreferrer" className="bg-slate-900 text-white px-10 py-6 rounded-full text-xl font-bold italic uppercase hover:bg-blue-600 transition-all flex items-center justify-center gap-4 mx-auto w-fit">
                      <Calendar size={24} /> Book Strategic Audit
                    </a>
                  </div>
                  <button onClick={() => setActiveFilter(null)} className="block mx-auto text-slate-400 font-bold uppercase tracking-widest hover:text-blue-600 transition-colors">Return to Portfolio</button>
                </motion.div>
              )}
            </div>
          </motion.div>
