...
export default function Home() {
  ...
  // 1. Initial State Object for easy clearing
  ...
  // 2. Updated handleSendEmail with State Reset
  const handleSendEmail = () => {
    ...
    // Trigger the email draft
    window.location.href = `mailto:${emailAddress}?subject=${subject}&body=${body}`;
    
    // Manually clear the form to indicate completion
    setFormData(initialFormState);
    
    // Optional: Add a subtle feedback alert
    console.log("Strategy Request Generated. Form Cleared.");
  };
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100">
      {/* Navigation Header */}
      <nav className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 font-bold">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-2 px-5 py-2 rounded-full hover:bg-slate-50 transition-all text-slate-500 hover:text-blue-600 border border-slate-100"
            >
          ...
          <button onClick={() => navigateTo('intake')} className="bg-blue-600 text-white px-5 py-2.5 rounded-full hover:bg-blue-700 transition-all flex items-center gap-2 shadow-lg shadow-blue-100 text-sm">
      ...
      <main className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        ...
        {currentView === 'casestudy' && <CaseStudyView />}
        {currentView === 'intake' && <IntakeView formData={formData} setFormData={setFormData} handleSendEmail={handleSendEmail} />}
      </main>

      <footer className="py-12 text-center border-t border-slate-50 italic text-slate-900">
  ...
  return (
    <div className="animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row gap-12 items-center md:items-start mb-20">
        <div className="relative w-56 h-56 bg-white rounded-3xl overflow-hidden border-2 border-white shadow-2xl shrink-0"><Image src="/headshot.jpeg" alt="Headshot" fill className="object-cover" /></div>
        <div className="max-w-4xl text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-600 text-[10px] font-bold uppercase tracking-[0.2em] mb-8"><Sparkles className="text-blue-500" size={12} /> Strategic AI Implementation</div>
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-8 leading-[1.1] text-slate-900 italic">Building Intelligent <br /><span className="bg-gradient-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent italic underline decoration-blue-500/30">Agentic Engines.</span></h1>
          <p className="text-xl text-slate-500 max-w-2xl font-medium italic mb-10 leading-relaxed">Over 10 years of experience at <strong>Airloom AI, Amazon, AWS and Microsoft (Xbox, Blizzard)</strong> architecting autonomous systems to solve problems globally.</p>
...
function CapabilitiesView() {
  return (
    <div className="animate-in fade-in duration-500">
      <div className="bg-slate-900 rounded-[3rem] p-12 md:p-20 text-white flex flex-col lg:flex-row gap-16 items-center shadow-2xl border border-slate-800">
        <div className="flex-1">
          ...
          <p className="text-slate-400 text-xl leading-relaxed mb-10 font-light italic">I build autonomous systems that **Ingest** complex datasets, **Synthesize** intent, and **Execute** multi-step objectives without constant supervision.</p>
          <div className="flex flex-wrap gap-3 font-mono text-[10px] uppercase font-bold tracking-widest">{['React.js', 'OpenAI API', 'Vercel', 'Firebase'].map(t => <span key={t} className="px-4 py-1.5 bg-slate-800 rounded-lg border border-slate-700 text-blue-300">{t}</span>)}</div>
        </div>
        <div className="w-full lg:w-1/3 aspect-square bg-blue-600/10 rounded-[3rem] border border-blue-500/20 flex items-center justify-center relative"><Cpu size={140} className="text-blue-500 opacity-20 animate-pulse" /></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="animate-in fade-in duration-500 text-center">
      ...
      <div className="max-w-4xl mx-auto space-y-6 text-left">
        <ExperienceCard company="Blizzard Entertainment" role="Product Manager, Battle.net" date="2025-Present" bullets={["Strategy Owner for Video UX across franchises.", "Shipped gamepad support for Xbox handheld devices."]} isCurrent={true} />
        <ExperienceCard company="Amazon, AWS & Alexa" role="Product & Program Management" date="2017-2024" bullets={["AWS: Avoided $6M in software costs.", "Alexa Ads: Improved domain CTR to 1.8%."]} />
      </div>
    </div>
  );
}
...
function CaseStudyView() {
  const cases = [
    { t: '01. Entry & Identity', i: '/lilologin.png', d: 'Secure onboarding built on Firebase.' },
    { t: '02. Command & Control', i: '/lilo-triage-admin.png', d: 'Centralized autonomous dispatching.' },
    { t: '03. Contextual Solving', i: '/lilo-blocking-issue.png', d: 'Synthesizing blockers into data.' },
    { t: '04. Autonomous Action', i: '/lilo-employee-view.jpeg', d: 'Field-ready execution layer.' },
    { t: '05. Reliability Loops', i: '/lilo-employee-scorecard.png', d: 'Tracking performance for Sammy & Lucy.' },
    { t: '06. Operational Insights', i: '/lilo-dashboard-admin.png', d: 'Automated health monitoring.' }
  ];
  return (
    <div className="animate-in fade-in duration-500">
      <h2 className="text-5xl font-extrabold text-slate-900 mb-8 italic">LILO OS Case Study</h2>
      <p className="text-lg text-slate-500 mb-16 font-light italic">Applying enterprise rigor to personal infrastructure.</p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{cases.map((c, i) => (
        <div key={i} className="group rounded-3xl overflow-hidden border border-slate-100 bg-white hover:border-blue-200 transition-all">
          <div className="relative h-64 bg-slate-900"><Image src={c.i} alt={c.t} fill className="object-cover opacity-90 group-hover:scale-105 transition-transform" /></div>
          <div className="p-8"><h3 className="font-bold text-lg italic">{c.t}</h3><p className="text-sm text-slate-400 mt-2">{c.d}</p></div>
        </div>
      ))}</div>
    </div>
  );
    ...
    <div className="animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <form className="space-y-8 bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100">
        ...
        <div className="space-y-2">
          ...
          <textarea className="w-full px-8 py-5 bg-slate-50 rounded-3xl border border-transparent focus:bg-white focus:border-blue-500 outline-none transition-all resize-none font-light" value={formData.description} rows={4} placeholder="Describe the manual workflow or process you want to automate..." onChange={(e) => setFormData({...formData, description: e.target.value})} />
        </div>
        <div className="space-y-2">
          ...
          <textarea className="w-full px-8 py-5 bg-slate-50 rounded-3xl border border-transparent focus:bg-white focus:border-blue-500 outline-none transition-all resize-none font-light" value={formData.outcome} rows={3} placeholder="What is the project goal or autonomous outcome you want to achieve?" onChange={(e) => setFormData({...formData, outcome: e.target.value})} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            ...
            <select className="w-full px-8 py-5 bg-slate-50 rounded-3xl border border-transparent outline-none appearance-none cursor-pointer" value={formData.timeline} onChange={(e) => setFormData({...formData, timeline: e.target.value})}>
...
function DropdownItem({ icon: Icon, label, sub, onClick }: any) {
  return (
    <button onClick={onClick} className="w-full flex items-center gap-4 p-4 hover:bg-slate-50 rounded-xl transition-all group text-left">
      ...
      <div><div className="text-sm font-bold text-slate-900">{label}</div><div className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{sub}</div></div>
    </button>
  );
}
...
function Input({ label, placeholder, value, onChange, color = "text-slate-400" }: any) {
  return (
    <div className="space-y-2">
      ...
      <input type="text" placeholder={placeholder} value={value} className="w-full px-8 py-5 bg-slate-50 rounded-3xl border border-transparent focus:bg-white focus:border-blue-500 outline-none transition-all font-light" onChange={(e) => onChange(e.target.value)} />
    </div>
  );
}
